...
async function getSectionData() {
  const section = await client.fetch(SECTION_QUERY, { slug: "anerkjennende-kommunikasjon" });
  return section;
}

export const revalidate = 30;

export default async function AnerkjennendekommunikasjonPage() {
        ...
        <ol className="flex items-center gap-2 text-sm">
          ...
          <li className="flex items-center gap-2">
            ...
            <span className="text-[var(--current-theme-color-900)] font-medium">
              {section?.title || "Anerkjennende kommunikasjon"}
            </span>
          </li>
        </ol>
      </nav> */}
      ...
      <header className="mb-10 animate-fade-in-up">
        <div className="flex items-start gap-4">
          <div className="flex-shrink-0 w-14 h-14 rounded-2xl bg-[var(--current-theme-color-200)] flex items-center justify-center">
            <Icon icon={section?.iconName || "heart"} size={28} strokeWidth={1.5} className="" />
          </div>
          <div>
      ...
      {section?.content ? (
        ...
        <div className="py-12 text-center animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[var(--current-theme-color-200)] mb-4">
            <Icon icon="heart" size={28} className="text-[var(--current-theme-color-800)]" />
          </div>
          <Body1 className="">
